...
describe('OfficialDataApp', () => {
  ...
  it('shows official provenance, freshness, authority, and source validation summary', async () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date('2026-08-28T11:00:00.000Z'));
    vi.stubGlobal(
      'fetch',
    ...
    expect(screen.getByRole('heading', { name: '데이터 근거' })).toBeInTheDocument();
    expect(
      screen.getByRole('link', { name: '행정안전부 전국생활쓰레기배출정보표준데이터' }),
    ).toHaveAttribute('href', 'https://www.data.go.kr/data/15025450/standard.do');
    ...
    expect(screen.getByText(/가져온 시각/)).toBeInTheDocument();
  });
